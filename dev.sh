...
GREEN='\033[00;32m'
YELLOW='\e[0;33m'
HOST_PROD=exemplo_djavue2.example.com
...
function dkbuild {
    CD=$(pwd)
    cd $PROJ_BASE
    docker build -t exemplo_djavue2 .
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development exemplo_djavue2 npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop exemplo_djavue2
    docker rm exemplo_djavue2
    docker run --name exemplo_djavue2 -d --env-file /home/ubuntu/exemplo_djavue2.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/exemplo_djavue2:/dkdata \
        exemplo_djavue2 start_web.sh
}

function deploy_prod {
  rsync -av --exclude frontend/node_modules --exclude frontend/.nuxt * ubuntu@$HOST_PROD:./exemplo_djavue2/
...
function dk {
    docker exec -it exemplo_djavue2 $@
}
...
echo_green "Welcome to exemplo_djavue2's dev env"
