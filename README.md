...
```bash
dk bash  # starts bash inside "exemplo_djavue2" container
...
* `dkup` uses docker-compose to start 3 containers: postgres, nginx, and exemplo_djavue2.
...
* Once `dkup` is running, `dk <command>` will run `<command>` inside the `exemplo_djavue2` container. So `dk bash` will get you "logged in" as root inside that container. Once inside, you need to run Django's `manage.py` commands to initialize the database properly.
* The exemplo_djavue2 container runs 3 services:
 * django on port 8000
...
```bash
mkvirtualenv exemplo_djavue2 -p python3  # creates a python3 virtualenv
...
Install docker and nginx. Create an empty postgres database exemplo_djavue2 owned by a user exemplo_djavue2.

On your remote machine, create a file ~/exemplo_djavue2.env:

```
DJANGO_DB_PASSWORD=<exemplo_djavue2's password>
DJANGO_DB_HOST=<database_ip>
DJANGO_DB_NAME=exemplo_djavue2
DJANGO_DB_USER=exemplo_djavue2
DJANGO_DEBUG=0
```
...
```
server {
    server_name  exemplo_djavue2.example.com;
    ...
    location /static {
        alias /home/ubuntu/dkdata/exemplo_djavue2/static;
...
(Replace "exemplo_djavue2.example.com" with your production domain)
